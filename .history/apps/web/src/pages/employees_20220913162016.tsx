  ...
  return (
    <div className="overflow-x-auto relative shadow-md sm:rounded-lg m-10">
      ...
      <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          {data?.employees?.edges.map((edge: { node: EmployeeType }) => {
            const dob = new Date(edge.node.dateOfBirth);
            return (
              <tr
                key={edge.node.id}
              ...
              >
                <td className="py-4 px-6">
                  <div className="flex items-center">{edge.node.firstName}</div>
                </td>
                <td className="py-4 px-6">
                  <div className="flex items-center">{edge.node.lastName}</div>
                </td>
                <td className="py-4 px-6">
                  <div className="flex items-center">{edge.node.phoneNo}</div>
                </td>
                <td className="py-4 px-6">
                  <div className="flex items-center">{edge.node.email}</div>
                </td>
                <td className="py-4 px-6">
                  <div className="flex items-center">{edge.node.gender}</div>
                </td>
                <td className="py-4 px-6">
                  <div className="flex items-center">
                    {dob?.getUTCDate()} / {dob?.getMonth()} /{' '}
                    {dob?.getFullYear()}
                  </div>
                </td>
                <td className="py-4 px-6">
                  <div className="flex items-center">{edge.node.country}</div>
                </td>
                <td className="py-4 px-6">
                  <a
                    href="/"
                  ...
                  <button
                    onClick={() => {
                      deleteEmployee({ variables: { id: edge.node.id } });
                    }}
                    className="ml-5 font-medium text-red-600 dark:text-blue-500 hover:underline"
                  >
                    Delete
                  </button>
                </td>
