...
import { ADD_DEPARTMENT, DELETE_DEPARTMENT } from '../graphql/mutations';
...
export default function Departments() {
  ...
  const [
    addDepartment,
    { data: dataDep, loading: loadingDep, error: errorDep },
  ...
  useEffect(() => {
    if (data != null) {
      if (searchTerm === '') {
        setFilteredItems(data?.departments?.edges);
      } else {
        setFilteredItems(
          data?.departments?.edges.filter(
            (edge: { node: DepartmentType }) =>
              edge.node.name.toLowerCase() === searchTerm.toLowerCase()
          )
        );
      }
    }
  ...
  return (
    <div className="overflow-x-auto relative shadow-md sm:rounded-lg m-10">
              ...
              <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div className="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600">
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      addDepartment({ variables: { name: name } });
                    }}
                    className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                  >
                    {loadingDep ? 'Loading...' : 'Add Department'}
                  </button>
                  <button
                    data-modal-toggle="defaultModal"
                    type="button"
      ...
      <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          {filteredItems.map((edge: { node: DepartmentType }) => {
            return (
              <tr
                key={edge.node.id}
              ...
              >
                ...
                <td className="py-4 px-6">
                  ...
                  <button
                    onClick={() => {
                      deleteDepartment({ variables: { id: edge.node.id } });
                    }}
